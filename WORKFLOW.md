...
- **Code coverage:** 87% overall, 100% for fir/lpf/media_noise
- **Experiments:** 14/14 passed
- **Total runtime:** ~78 seconds
- **C/Python comparison:** BER within same order of magnitude across SNR range

### C Code Fixes (2026-05-22)

1. **`void main` → `int main`** (line 3356) + added `return 0`
2. **Pointer type mismatches**: `double *Hc = &(mp->Hc)` → `mp->Hc` (lines 2462, 2724)
3. **`int *` → `long *`** for RNG seed variables (lines 2738-2739)
4. **`sizeof(int)` → `sizeof(int*)`** for pointer array mallocs in CustomDetectors.c
5. **`&` vs `==` precedence**: `((ModulationValues[i] >> j) & 1) == 1` (lines 2500, 2530, 2558)
6. **`NumEqSectors = 0` → `10`** to enable equalizer adaptation

C code now runs successfully: SNR 30dB, BER = 2.34e-04 (Longitudinal, EPR4, Viterbi).

### Python Fixes (2026-05-22)

1. **`equalizer.py` signature mismatch**: Removed duplicate `non_causal_fir`/`causal_fir`, now imports from `channel.fir`
2. **`media_noise.py`**: Full implementation of C `MediaNoiseFilter()` (was 17% stub)
3. **BER comparison bug**: Fixed to compare `detected[pre_pad:pre_pad+sector_length]` with `encoded` (was comparing wrong regions)
4. **FixedPRTarget LMS adaptation**: Added missing equalizer training phase (was using zero coefficients)
5. **Dead code removal**: Removed unreachable `return` in `channel.py`

### Known Issues (2026-05-22)
